...
class BertWithCLIPStyleFusion(BertPreTrainedModel):
    def __init__(self, config, feature_dim=10, fusion_hidden_dim=128):
        super().__init__(config)
        self.num_labels = config.num_labels
        self.bert = BertModel(config)

        # Project numerical features if needed
        ...
        # Fuse DNABERT and feature embeddings (concatenate + projection)
        self.fusion_layer = nn.Linear(config.hidden_size + fusion_hidden_dim, config.hidden_size)
    ...
    def forward(
        ...
        attention_mask=None,
        token_type_ids=None,
        features=None,  # [batch_size, feature_dim]
        labels=None
    ):
        # DNABERT sequence encoding
        outputs = self.bert(
            input_ids=input_ids,
        ...
        # Expand feature tensor: [B, F] -> [B, seq_len, F]
        projected_features = self.feature_proj(features)  # [B, fusion_hidden_dim]
        expanded_features = projected_features.unsqueeze(1).expand(-1, seq_len, -1)  # [B, seq_len, fusion_hidden_dim]

        # Fuse sequence and features: concat + projection
        fused = torch.cat([sequence_output, expanded_features], dim=-1)  # [B, seq_len, H + F]
        fused = self.fusion_layer(fused)  # [B, seq_len, H]

        # Use [CLS] token for classification
        cls_output = fused[:, 0]  # [B, H]
