...
import wandb
import pandas as pd
# os.environ['WANDB_DIR'] = '/data/private/npapineni/wandb_temp'
...
def train(args, train_dataset, model, tokenizer, log_file_name):
    """ Train the model """
    if args.local_rank in [-1, 0]:
        tb_writer = SummaryWriter(log_file_name)

    args.train_batch_size = args.per_gpu_train_batch_size * max(1, args.n_gpu)
    train_sampler = RandomSampler(train_dataset) if args.local_rank == -1 else DistributedSampler(train_dataset)
    train_dataloader = DataLoader(train_dataset, sampler=train_sampler, batch_size=args.train_batch_size)
    #print ("@@##", args.max_steps, args.gradient_accumulation_steps, args.num_train_epochs)

    if args.max_steps > 0:
        t_total = args.max_steps
        args.num_train_epochs = args.max_steps // (len(train_dataloader) // args.gradient_accumulation_steps) + 1
        #print("test", args.max_steps, len(train_dataloader), args.num_train_epochs, args.gradient_accumulation_steps)
    else:
        t_total = len(train_dataloader) // args.gradient_accumulation_steps * args.num_train_epochs
        # print("test1", args.max_steps, len(train_dataloader), args.num_train_epochs, args.gradient_accumulation_steps, t_total)
        # input()
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if args.local_rank in [-1, 0] and args.logging_steps > 0 and global_step % args.logging_steps == 0:
                    ...
                    if (
                        ...
                        results, confusion_matrix = evaluate(args, model, tokenizer, global_step)
                    ...
                    wandb_log = logs
                    wandb_log["Confusion Matrix"] = wandb.Image(confusion_matrix)
...
def evaluate(args, model, tokenizer, global_step , prefix="", evaluate=True):
    ...
    eval_outputs_dirs = (args.summary_dir, args.summary_dir + "-MM") if args.task_name == "mnli" else (args.summary_dir,)
    if args.task_name[:3] == "dna":
        softmax = torch.nn.Softmax(dim=1)
        

    results = {}
    # print(eval_task_names , eval_outputs_dirs)
    # input()
    for eval_task, eval_output_dir in zip(eval_task_names, eval_outputs_dirs):
        additional_val_df = pd.read_csv(args.data_dir+"/"+args.additional_feature+"_dev.tsv" , sep="\t")
        print(additional_val_df.shape)
        #input()
        #additional_val_feature = torch.tensor(additional_val_df.values).long()
        eval_dataset = load_and_cache_examples(args, eval_task, additional_val_df, tokenizer, evaluate=evaluate)

        if not os.path.exists(eval_output_dir) and args.local_rank in [-1, 0]:
            os.makedirs(eval_output_dir)
            # print("eval", eval_output_dir)
            # input()
        ...
        result['Validation loss'] = eval_loss
    ...
    #print(global_step, results, args)
    confusion_matrix = confusion_matrix_generation(args, eval_output_dir, preds, out_label_ids)
    if args.do_ensemble_pred:
...
def predict(args, model, tokenizer, prefix=""):
    ...
    for pred_task, pred_output_dir in zip(pred_task_names, pred_outputs_dirs):
        # print (pred_task_names, pred_outputs_dirs)
        # print (pred_task, pred_output_dir)
        # exit()
        # input()
        additional_pred_df = pd.read_csv(args.data_dir+"/Evaluation_data/"+args.additional_feature+"_dev.tsv" , sep="\t")
        print(additional_pred_df.shape)
        #additional_pred_feature = torch.tensor(additional_pred_df.values).long()
        pred_dataset = load_and_cache_examples(args, pred_task, additional_pred_df, tokenizer, evaluate=True)

        if not os.path.exists(pred_output_dir) and args.local_rank in [-1, 0]:
            os.makedirs(pred_output_dir)

        args.pred_batch_size = args.per_gpu_pred_batch_size * max(1, args.n_gpu)
        print("Prediction batch size : ", args.pred_batch_size)
        # Note that DistributedSampler samples randomly
        ...
        for batch in tqdm(pred_dataloader, desc="Predicting"):
            ...
            with torch.no_grad():
                ...
                outputs = model(**inputs)
                #print ("outputs",len(outputs[0]), len(outputs[1]))
                _, logits = outputs[:2]
            ...
            if preds is None:
                ...
                #input()
                out_label_ids = inputs["labels"].detach().cpu().numpy()
                #print ("Preds12", len(out_label_ids), inputs["labels"], out_label_ids)
            else:
                preds = np.append(preds, logits.detach().cpu().numpy(), axis=0)
                out_label_ids = np.append(out_label_ids, inputs["labels"].detach().cpu().numpy(), axis=0)
                #print ("Preds13", preds, len(out_label_ids), out_label_ids)
        ...
        if args.do_ensemble_pred:
            ...
            print("Label", out_label_ids)
            print(probs)
            result = compute_metrics(pred_task, preds, out_label_ids, probs)
            # print ("PRED", preds, len(preds))
            # print("PROBS", probs, len(probs))
            # print("OUT_LABEL_IDS", out_label_ids, len(out_label_ids))
            # print("gr")
        ...
        output_pred_label_file = os.path.join(pred_output_dir, "predicted_labels.txt")
        logger.info("***** Pred results {} *****".format(prefix))
        for key in sorted(result.keys()):
            logger.info("  %s = %s", key, str(result[key]))
        np.save(output_pred_file, probs)
        np.save(output_pred_label_file, preds)
        
        ## Saving the prediction into WandB Table
        dev_data = pd.read_csv(args.data_dir+"/dev.tsv", sep='\t' )
        seq_list = dev_data['Sequence'].to_list()
        # wand_data = [probs, preds, out_label_ids]
        # wand_Table(data=wand_data)
        
        ## Uncomment if you want to add wandtable or run run_finetune_WAND_table.py
        # for i in range(len(seq_list)):
        #     wand_Table.add_data(seq_list[i], probs[i], preds[i], out_label_ids[i])
        conf_fig = confusion_matrix_generation(args, pred_output_dir, preds, out_label_ids)

        
def confusion_matrix_generation(args, output_path, predicted_labels, actual_labels):
    global flag  
    flag += 1
    cf_matrix = confusion_matrix(actual_labels, predicted_labels)
    print ("Confusion matrix: ",cf_matrix)
    group_names = ['True Neg','False Pos','False Neg','True Pos']
    labels = ["{0:0.0f}".format(value) for value in cf_matrix.flatten()]
    labels = np.asarray(labels).reshape(2,2)
    plt.figure(figsize = (12,12))
    class_accuracy = 100*cf_matrix.diagonal()/cf_matrix.sum(1)
    ax = sn.heatmap(cf_matrix, annot=labels, fmt='')
    # print("Accuracy class 1: ", class_accuracy[1])
    # print("Accuracy class 0: ", class_accuracy[0])
    ...
    # input()
    title="Accuracy for for predicting {} [class 1] =".format(args.classes_name.split("_")[0])+ str(class_accuracy[1])+"\n" \
            "Accuracy for for predicting {} [class 0] =".format(args.classes_name.split("_")[1])+ str(class_accuracy[0])+"\n" \
            "Overall Accuracy: {:.3f}; ".format(accuracy_score(actual_labels, predicted_labels)) +\
            "Precision: {:.3f}; ".format(precision_score(actual_labels, predicted_labels, average="macro")) + \
            "Recall: {:.3f}; ".format(recall_score(actual_labels, predicted_labels, average="macro")) +\
            "F-score:  {:.3f}.".format(f1_score(actual_labels, predicted_labels, average="macro"))+"\n\n"
    ...
    name= output_path.split("/")[-2]+ '_confusion_matrix_'+str(flag)+'.png'
    confusion_matrix_folder_path = os.path.join(output_path, 'confusion_matrix_images')
    if not os.path.exists(confusion_matrix_folder_path):
        os.makedirs(confusion_matrix_folder_path)
    output_confusion_image = os.path.join(confusion_matrix_folder_path, name)
    plt.savefig(output_confusion_image)
...
def visualize(args, model, tokenizer, kmer, prefix=""):
    ...
    for pred_task, pred_output_dir in zip(pred_task_names, pred_outputs_dirs):
        ...
        print(additional_pred_df.shape)
        #additional_pred_feature = torch.tensor(additional_pred_df.values).long()
        pred_dataset = load_and_cache_examples(args, pred_task, additional_pred_df, tokenizer, evaluate=evaluate)
        ...
        attention_scores = np.zeros([len(pred_dataset), 12, args.max_seq_length, args.max_seq_length], dtype=np.float32)
...
def load_and_cache_examples(args, task, additional_feature_df,  tokenizer, evaluate=False):
    ...
    # print(all_input_ids.size(), all_attention_mask.size(), all_token_type_ids.size())
    # print(all_attention_mask[0])
    # input()
    #print(all_input_ids[0])
    # print(torch.max(all_input_ids).item())
    # input()
    # print(all_token_type_ids[:5])
    #input()
    #all_input_ids  = torch.cat((all_input_ids, additional_feature), dim=1)
    #additional_feature *= 1000
    #print(additional_feature_df.iloc[0].tolist())
    #input()
    
    ############################################################################ ADDITIONAL FETURE COMBINING #####################################################
    ## Scaling the additional feature based by multipled by 1000
    #additional_feature = ((torch.tensor(additional_feature_df.values)+1)*1000).long()
    
    # Scaling the additional feature based by  arctangent(atan) function
    atan_val = torch.atan(torch.tensor(additional_feature_df.values))
    scaled_val = (atan_val + np.pi/4) / np.pi/2
    additional_feature  = ((scaled_val * 1000)+10).long()
    
    ##############################################################################################################################################################
    
    # all_input_ids  = torch.cat([all_input_ids[:, :-(add_feature_length+4)], additional_feature_1, all_input_ids[:, -(add_feature_length+4):-add_feature_length]], dim=1)
    
    
    
    ##Changing the all_inputs_ids and attention ids by concating 
    add_feature_length = additional_feature.size(1)
    all_input_ids  = torch.cat([all_input_ids[:, :-(add_feature_length+4)], additional_feature, all_input_ids[:, -(add_feature_length+4):-add_feature_length]], dim=1)
    # all_attention_mask = torch.cat((all_attention_mask, torch.ones((additional_feature.size(0), additional_feature.size(1)), dtype=torch.float)) , dim=1)
    all_attention_mask[:, -(add_feature_length+4):-3]=1
    #print(additional_feature.size(0) , additional_feature.size(1))
    #print(all_input_ids.size(), all_attention_mask.size(), all_token_type_ids.size())
    #print(all_attention_mask[0])
    #input()
    #print(all_input_ids[0])
    print("Checking the additional feature size")
    print("Normalizing the feature vector")
    #input()
    ## NORMALIZING THE VALUE
#     mean_val = torch.mean(all_input_ids, dim=0, keepdim=True)
#     std_val = torch.std(all_input_ids, dim=0, keepdim=True)
...
#     normalized_tensor = (all_input_ids - mean_val) / torch.where(mask_std_zero, torch.ones_like(std_val), std_val)

#     # Assign original values to columns with std=0
#     normalized_tensor[:, mask_std_zero.squeeze()] = all_input_ids[:, mask_std_zero.squeeze()]
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="The name of the classes for the classification task",
    )
    parser.add_argument(
        "--additional_feature",
        default=None,
        type=str,
        required=True,
        help="The additional feature name for the classification task",
    )
    parser.add_argument(
        "--wandb_tags",
        default=None,
        ...
        type=str,
        required=True,
        help="Put the tags of WANDB for your Model",
    )
    parser.add_argument(
        "--architecture",
        default=None,
        type=str,
        required=True,
        help="The name of the model name in which the model is saved in WANDB profile",
    )
    parser.add_argument(
        "--output_dir",
        default=None,
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="The output directory where the TensorBoard files are stored.",
    )
    parser.add_argument(
        "--summary_dir",
        default=None,
        type=str,
        required=True,
        help="The output directory where the summary of each epoch and the confusion of each validation are stored.",
    )
    ...
    print ("batch_size", args.per_gpu_train_batch_size * max(1, args.n_gpu))
    print("warm", args.warmup_percent)
    print("dropout", args.hidden_dropout_prob)
    print("max_seq", args.max_seq_length)
    hyperameter_list =  "logstep="+str(args.logging_steps)+"_bs="+str(args.per_gpu_train_batch_size * max(1, args.n_gpu))+"_lr="+str(args.learning_rate)+"_wp="+str(args.warmup_percent)+"_dp="+str(args.hidden_dropout_prob)+"_wd="+str(args.weight_decay)+"_len="+str(args.max_seq_length)+"_epoch="+str(args.num_train_epochs)
    print("Hyperparameter details: ", hyperameter_list)
    
    ## Get the current time and date for the log filename
    now = datetime.datetime.now()
    day_str = now.strftime('%d')  # Get the day number as a string
    if day_str.endswith(('1', '2', '3')):
        # For days ending with 1, 2, or 3, append 'th' instead of the usual suffix
        day_str += 'th'
    else:
        # For other days, append the corresponding suffix (st, nd, or rd)
        day_str += { '01': 'st', '02': 'nd', '03': 'rd' }.get(day_str[-2:], 'th')
    date_str = f"{now.strftime('%B')}_{day_str}_{now.strftime('%Y')}"
    time_str = now.strftime('%H_%M_%S')
    current_date = f"{date_str}_{time_str}"
    # print(current_date)
    # input()

    if not os.path.exists(args.tb_log_dir) and args.local_rank in [-1, 0]:
        os.makedirs(args.tb_log_dir)
    
    if not os.path.exists(os.path.join(args.summary_dir, hyperameter_list)) and args.local_rank in [-1, 0]:
        #print(os.path.join(args.summary_dir, hyperameter_list))
        os.makedirs(os.path.join(args.summary_dir, hyperameter_list))
    log_file_name = args.tb_log_dir+'/'+current_date+"_"+hyperameter_list
    print("TB_log file path: ", log_file_name)
    # input()
    ...
    wandb.init(
		...
		project= args.architecture+"_Work",
        entity="nimisha_sbu",
        tags= args.wandb_tags,
		name=args.classes_name+"_"+current_date+"_"+hyperameter_list,
	)
    cfg = wandb.config
    ...
    cfg.update({
		...
		"warm up percentage": args.warmup_percent,
		"Maximum sequence length": args.max_seq_length,
		"Dropout": args.hidden_dropout_prob,
		"Weight Decay":args.weight_decay,
		})
    # columns=["Probability", "Predicted_label", "True_label"]
    
    ## Uncomment if you want to add wandtable or run run_finetune_WAND_table.py
    #wand_Table = wandb.Table(columns=["K-mer","Probability", "Predicted_label", "True_label"])


    # Setup logging
    logging.basicConfig(
        filename=os.path.join(args.summary_dir, hyperameter_list)+'/summary.log',
    ...
    if args.do_eval:
        args.output_dir = os.path.join(args.output_dir, hyperameter_list)
        args.summary_dir = os.path.join(args.summary_dir, hyperameter_list)
    ...
    # Training
    if args.do_train:
        additional_train_df = pd.read_csv(args.data_dir+"/"+args.additional_feature+"_train.tsv" , sep="\t")
        print(additional_train_df.shape)
        #input()
        #additional_train_feature = (torch.tensor(additional_train_df.values)*1000).long()
        train_dataset = load_and_cache_examples(args, args.task_name, additional_train_df, tokenizer, evaluate=False)
        global_step, tr_loss = train(args, train_dataset, model, tokenizer, log_file_name)
    ...
    if args.do_eval and args.local_rank in [-1, 0]:
        ...
        for checkpoint in checkpoints:
            ...
            model.to(args.device)
            result, conf_matrix = evaluate(args, model, tokenizer, global_step, prefix=prefix)
    ...
    if args.do_predict and args.local_rank in [-1, 0]:
        ...
        ## Uncomment if you want to add wandtable or run run_finetune_WAND_table.py
        #wandb.log({"Prediction Result":wand_Table})
    ...
    if args.do_visualize and args.local_rank in [-1, 0]:
        ...
        atten_score = "atten_score"+str(args.max_seq_length)+".npy"
        np.save(os.path.join(args.predict_dir, atten_score), scores)
